...
gbr = joblib.load('linear_five.pkl')
...
def get_delay():
    result=request.form
    mrpbox = int(result['mrpbox'])
    mrpsqm = int(result["mrpsqm"])
    total_ad = int(result["total_ad"])
    billing_rate = int(result['billing_rate'])
    buyer_rate = int(result['buyer_rate'])

    data = [mrpbox,mrpsqm,total_ad,billing_rate,buyer_rate]
    print(data)
    price_pred = gbr.predict([data])[0]
